...
// Switch Pro 2 Input Report (0x09) - 62 bytes
typedef struct {
  uint8_t  report_id;   // 0x09
  uint8_t  counter;     // Sequence counter
  uint8_t  power_info;  // Battery/power status

  // 3-byte button field (need to verify bit mapping)
  struct {
    uint8_t y    : 1;
    uint8_t x    : 1;
    uint8_t b    : 1;
  ...
  uint8_t  unknown;         // Always 0x38
  uint8_t  unused;
  uint8_t  headset_flags;
  uint8_t  motion_length;
  uint8_t  motion_data[40]; // Variable length motion data
  uint8_t  reserved[8];

  uint16_t left_x, left_y, right_x, right_y;
} switch_pro2_report_t;
